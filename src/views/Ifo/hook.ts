    ...
    if(account){
      ...
      return multicallProvider
        ...
        .then((data) => {
          ...
          if (
            totalVolume.toString() === totalSettledUnderlying.toString() &&
            totalVolume > 0
          ) {
            status = 3
          }
          ...
          const totalPurchasedAmount = new BigNumber(
            toWei(ifo.amount, 'ether').toString()
          )
          ...
          // @ts-ignore
          const progress:number = new BigNumber(totalPurchasedCurrency)
              .dividedBy(totalPurchasedAmount)
              .toFixed(2, 1)
              .toString() * 1

          setInfo({ ...ifo, ratio: `1${ifo.underlying.symbol}=${formatAmount(
              price,
              18,
              5
            )}${ifo.currency.symbol}`,
